...
const CoinChange = ({ problem }) => {
    ...
    const steps = useMemo(() => {
        ...
        record('start', {
            description: `Initialize DP array of size ${amount + 1}. dp[0] = 0, others = Infinity.`,
            activeCoin: null,
            activeIndex: null,
            prevIndex: null
        });
        ...
        for (let i = 1; i <= amount; i++) {
            record('check_index', {
                description: `Calculating minimum coins for amount ${i}.`,
                activeIndex: i,
                activeCoin: null
            });
            ...
            for (const coin of coins) {
                if (i - coin >= 0) {
                    ...
                    const canMake = dp[prevAmount] !== Infinity;

                    record('check_coin', {
                        description: `Try coin ${coin}: Look at dp[${i} - ${coin}] = dp[${prevAmount}].`,
                        activeIndex: i,
                        activeCoin: coin,
                    ...
                    if (canMake) {
                        const newCount = dp[prevAmount] + 1;
                        if (newCount < dp[i]) {
                            dp[i] = newCount;
                            record('update', {
                                description: `Found fewer coins! dp[${i}] = dp[${prevAmount}] + 1 = ${newCount}.`,
                                activeIndex: i,
                                activeCoin: coin,
                                prevIndex: prevAmount,
                                highlight: [i, prevAmount]
        ...
        record('finish', {
            description: `Finished! Minimum coins needed: ${dp[amount] === Infinity ? -1 : dp[amount]}`,
            activeIndex: amount,
            activeCoin: null
        });
